...
function buildMemberNav_changelog(items, itemHeading, itemsSeen, linktoFn) {
    ...
    if (items && items.length) {
        ...
        items.forEach(function(item) {
			...
			var tmpObj = getChangelogsObj(item, item.changelog);
		...
		_.forEach(dataObj, function (value, key) {
			...
			datetime = strftime("%Y-%m-%d", new Date(datetime));
			var desc = value.desc;
...
function buildMemberNav_todolists(items, itemHeading, itemsSeen, linktoFn) {
    var nav = '';
    ...
    if (items && items.length) {
        ...
        items.forEach(function(item) {
			var kind = item.kind;
			var longname = item.longname;
			var tmpObj = getTodoListsObj(item);
			dataObj = _.union(dataObj, tmpObj);
        });
        ...
        if (itemsNav !== '') {
            nav += '<h3>' + itemHeading + '</h3><ul class="todolists-nav">' + itemsNav + '</ul>';
        }
    }
...
exports.publish = function(taffyData, opts, tutorials) {
	...
	var changelogs = taffy(members.changelogs); // custom nav
	var fixmes = taffy(members.fixmes); // custom nav
	var todolists = taffy(members.todolists); // custom nav
    ...
    Object.keys(helper.longnameToUrl).forEach(function(longname) {
		...
		// custom nav
		var myChangelogs = helper.find(changelogs, {longname: longname});
        if (myChangelogs.length) {
            generate('Changelog', myChangelogs[0].name, myChangelogs, helper.longnameToUrl[longname]);
        }

		// custom nav
		var myTodolists = helper.find(todolists, {longname: longname});
        if (myTodolists.length) {
            generate('Todolists', myTodolists[0].name, myTodolists, helper.longnameToUrl[longname]);
        }

		// custom nav
		var myFixmes = helper.find(fixmes, {longname: longname});
        if (myFixmes.length) {
            generate('Fixmes', myFixmes[0].name, myFixmes, helper.longnameToUrl[longname]);
        }
